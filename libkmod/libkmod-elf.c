...
enum kmod_elf_section {
	...
	KMOD_ELF_SECTION_VERSION_NAMES,
	KMOD_ELF_SECTION_VERSION_CRCS,
...
static const char *const section_name_map[] = {
	...
	[KMOD_ELF_SECTION_VERSION_NAMES] = "__version_ext_names",
	[KMOD_ELF_SECTION_VERSION_CRCS] = "__version_ext_crcs",
};
					     ...
					     struct kmod_modversion **array)
{
	...
	name_off = elf->sections[KMOD_ELF_SECTION_VERSION_NAMES].offset;
	name_size = elf->sections[KMOD_ELF_SECTION_VERSION_NAMES].size;
	crc_off = elf->sections[KMOD_ELF_SECTION_VERSION_CRCS].offset;
	crc_size = elf->sections[KMOD_ELF_SECTION_VERSION_CRCS].size;
				    ...
				    struct kmod_modversion **array)
{
	...
	version_array = NULL;
	vercount = kmod_elf_get_modversions(elf, &version_array);
	if (vercount < 0) {
		if (vercount == -ENODATA)
