...
struct kmod_list *kmod_list_append(struct kmod_list *list, const void *data) _must_check_
__attribute__((nonnull(2)));
struct kmod_list *kmod_list_prepend(struct kmod_list *list, const void *data) _must_check_
__attribute__((nonnull(2)));
...
struct kmod_list *kmod_list_remove_data(struct kmod_list *list, const void *data)
	_must_check_ __attribute__((nonnull(2)));
...
#define kmod_list_foreach(list_entry, first_entry)                                      \
	for (list_entry = ((first_entry) == NULL) ? NULL : (first_entry);               \
	     ...
	     list_entry = (list_entry->node.next == &((first_entry)->node)) ?           \
				  NULL :                                                \
				  container_of(list_entry->node.next, struct kmod_list, \
					       node))
...
#define kmod_list_foreach_reverse(list_entry, first_entry)                               \
	for (list_entry =                                                                \
	     ...
	     list_entry = ((list_entry == first_entry) ?                                 \
				   NULL :                                                \
				   container_of(list_entry->node.prev, struct kmod_list, \
						node)))
...
int kmod_config_new(struct kmod_ctx *ctx, struct kmod_config **config,
		    const char *const *config_paths) __attribute__((nonnull(1, 2, 3)));
...
enum kmod_file_compression_type kmod_file_get_compression(const struct kmod_file *file)
	_must_check_ __attribute__((nonnull(1)));
...
int kmod_elf_get_strings(const struct kmod_elf *elf, const char *section, char ***array)
	_must_check_ __attribute__((nonnull(1, 2, 3)));
...
__attribute__((nonnull(1, 2)));
int kmod_elf_strip_section(struct kmod_elf *elf, const char *section) _must_check_
__attribute__((nonnull(1, 2)));
...
bool kmod_module_signature_info(const struct kmod_file *file,
				struct kmod_signature_info *sig_info) _must_check_
__attribute__((nonnull(1, 2)));
...
ssize_t kmod_builtin_get_modinfo(struct kmod_ctx *ctx, const char *modname,
				 char ***modinfo) __attribute__((nonnull(1, 2, 3)));
