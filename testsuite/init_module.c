 ...
 */
/* TODO: add simple validation of the args passed and remove the _always_unused_ workaround */
long init_module(void *mem, unsigned long len, _always_unused_ const char *args)
{
	const char *modname;
	struct kmod_elf *elf;
	struct mod *mod;
...
/* TODO: add simple validation of the flags passed and remove the _always_unused_ workaround */
int finit_module(const int fd, const char *args, _always_unused_ const int flags)
{
	int err;
	void *mem;
	unsigned long len;
